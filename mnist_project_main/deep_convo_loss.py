...
model = Sequential()
model.add(Conv2D(filters = 16, kernel_size = ksize, activation='relu',
                 input_shape = (28, 28, 1)))
...
model.add(Conv2D(filters = 32, kernel_size = ksize, activation='relu'))
model.add(BatchNormalization())
model.add(Conv2D(filters = 32, kernel_size = ksize, activation='relu'))
model.add(BatchNormalization())
model.add(MaxPool2D(strides=(2,2)))
...
# saving model
save_model("deep_convo_hinge", model)
